...
// "5 1/2" -> 5.5, "11 1/2" -> 11.5, "7" -> 7, "3.5" -> 3.5
const NUM_RE = /^\d{1,2}(\.\d+)?$/;
const FRAC_RE = /^(\d)\/(\d)$/;
const PK_RE = /^(pk|pick|pick'?em|even)$/i;

function isNumberish(s) {
  const t = (s || "").trim();
  return NUM_RE.test(t) || FRAC_RE.test(t) || PK_RE.test(t);
}
...
function parseWords(words, pageWidth) {
  ...
  const colOf = (x) => (x < mid ? 0 : 1);

  const spreadCells = [];
  const gameCells = [];
  for (const r of rows) {
    for (const c of splitCells(r)) {
      if (/teams?\s+on\s+bye/i.test(c.text)) continue;
      const teams = splitTeams(c.text);
      if (teams) {
        gameCells.push({ cell: c, teams, y: r.y, col: colOf((c.x0 + c.x1) / 2) });
        continue;
      }
      // a cell that is only number-ish tokens is a spread
      const toks = c.items.filter((i) => i.str.trim());
      if (toks.length && toks.every((i) => isNumberish(i.str))) {
        const value = toks.reduce((s, i) => s + tokenValue(i.str), 0);
        spreadCells.push({
          value,
          x: toks[0].x + (toks[toks.length - 1].x + toks[toks.length - 1].w - toks[0].x) / 2,
          y: r.y,
          col: colOf(toks[0].x),
          used: false,
        });
  ...
  // --- pair each matchup with the nearest spread ABOVE it in the same column ---
  const games = [];
  gameCells.sort((a, b) => b.y - a.y || a.cell.x0 - b.cell.x0);
  for (const g of gameCells) {
    const bx = boundaryX(g.cell);
    const cands = spreadCells
      .filter((s) => !s.used && s.col === g.col && s.y > g.y && s.y - g.y < 70)
